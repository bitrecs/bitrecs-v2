...
def create_transport_signature(wallet: Wallet, submission: MinerSubmission, payment_block_hash: str, payment_extrinsic_hash: str, payment_extrinsic_index: int, amt_rao: int, nonce: str) -> str:
    preamble = f"{submission.created_at}:{submission.github_account}:{submission.gist_id}:{submission.hotkey}:{payment_block_hash}:{payment_extrinsic_hash}:{payment_extrinsic_index}:{amt_rao}:{nonce}"
    preamble_bytes = preamble.encode('utf-8')
    return wallet.hotkey.sign(preamble_bytes).hex()

def verify_transport_signature(submission: MinerSubmission, transport_signature: str, payment_block_hash: str, payment_extrinsic_hash: str, payment_extrinsic_index: int, amt_rao: int, nonce: str) -> bool:
    submission_preamble = f"{submission.created_at}:{submission.github_account}:{submission.gist_id}:{submission.hotkey}:{payment_block_hash}:{payment_extrinsic_hash}:{payment_extrinsic_index}:{amt_rao}:{nonce}"
